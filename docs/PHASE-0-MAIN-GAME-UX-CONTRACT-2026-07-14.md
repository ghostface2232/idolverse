...
- 전역 강도는 매니저 운영 방침으로 명명하여 멤버별 지시와 구분한다.
...
- 전체 주간 현황은 `매니저 운영/상태 상세` 보조 시트로 이동한다.
...
매니저가 맡은 항목은 숨기지 않고 ‘매니저 운영안’으로 요약해 보여준다. 플레이어가 직접 정하지 않은 결과도 왜 그렇게 배정되었는지 이해할 수 있어야 한다.
...
| **이번 주** | 이번 주에 무엇을 결정하고 실행하는가? | 결정 덱, 매니저 운영안, 검토, 실행 |
...
|---|---|
| 대시보드 | 회사 |
| 트레이닝 | 이번 주의 결정/매니저 운영안 또는 멤버 상세 |
...
가장 가까운 주요 마감 또는 현재 투자사 목표 하나만 표시한다.
...
secondary action은 최대 1개다. 예: `매니저 운영안 보기`.
