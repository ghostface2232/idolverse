        ...
        <div className="mb-3 grid grid-cols-2 gap-2">
          <div className="rounded-2xl bg-surface-panel px-3 py-2.5 shadow-[var(--shadow-surface)]">
            <p className="flex items-center gap-1.5 text-[11px] text-text-muted">
              <HeartPulse
                className="size-3.5 text-action-secondary"
                aria-hidden="true"
              />
              팀 평균 컨디션
            </p>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {trainees.map((trainee) => {
            const needsAttention =
              trainee.injuryWeeks > 0 || trainee.condition < 50;
            ...
            return (
              <button
                ...
                type="button"
                className={[
                  "rounded-3xl bg-surface-panel p-3.5 text-left shadow-[var(--shadow-surface)] transition-transform duration-[var(--motion-press)] active:scale-[0.96]",
                  needsAttention ? "ring-2 ring-state-danger/40" : "",
                ].join(" ")}
                ...
                <div className="flex items-center gap-3">
                  <span className="relative shrink-0">
                    ...
                    {needsAttention ? (
                      <span
                        aria-hidden="true"
                        className="absolute -right-0.5 -top-0.5 size-3 rounded-full bg-state-danger ring-2 ring-surface-panel"
                      />
                    ) : null}
                  </span>
                  <div className="min-w-0 flex-1">
                    <h2 className="flex items-center gap-1.5 truncate text-[15px] font-semibold text-text-primary">
                      {trainee.name}
                      {maxPopularity > 0 &&
                      (trainee.popularity ?? 0) === maxPopularity ? (
                        <span
                    ...
                    <div className="mt-1 flex flex-wrap gap-1 text-[11px] text-text-muted">
                      {trainee.injuryWeeks > 0 ? (
                        <span className="rounded-md bg-state-danger/15 px-1.5 py-0.5 font-semibold text-state-danger">
                          부상 {trainee.injuryWeeks}주 · 회복 필요
                        </span>
                      ) : null}
                      {trainee.condition < 50 ? (
                        <span className="rounded-md bg-state-danger/15 px-1.5 py-0.5 font-semibold text-state-danger">
                          컨디션 저하 · 휴식 필요
                        </span>
                      ) : null}
