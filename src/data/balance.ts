 ...
 * 대응을 고른 주부터(보류·강행 옵션도 대응이다) 이 주 수만큼은 매니저가
 * 후속을 맡고, critical로 악화된 위기는 절반 주기로 앞당겨 재소환한다 —
 * 위기가 "이벤트"가 아니라 매주 나가는 배경 세금이 되는 것을 막는 페이싱
 * 장치다. 쿨다운은 카드 제시가 아니라 대응 시점에 시작하므로, 대응 없이
 * 주가 넘어간 위기(엔진 직접 호출 경로)는 다음 주 곧바로 다시 올라온다.
 ...
 * fullScorePoints로 나눠 0~100 지표로 만든다. 심사는 시상 주(50주차)에
 * 그 시점까지의 누적(1~49주차)을 읽고, 50~52주차 득점은 연초 리셋으로
 * 소멸한다 — 늦은 발매의 차트 런은 새해로 넘어간 구간(1주차~)만 이듬해
 * 심사에 실리고, 연말 3주의 득점은 어느 해에도 실리지 않는다. 발매
 * 시점이 "올해 시상에 얼마나 실리는가"를 그대로 결정한다.
