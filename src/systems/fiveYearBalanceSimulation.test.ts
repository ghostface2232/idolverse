...
describe("초보와 숙련 플레이어의 5년 폐루프 밸런스", () => {
  ...
  it("숙련의 시상식 역산 스케줄은 최소한 손해가 아니어야 한다", () => {
    // 2026-07 프로브 기록(차트 축 도입 전): 당시 시상 체계(50주차 public
    // 스냅샷 + 그 해 최고 품질)는 발매 타이밍을 보상하지 않았고, 심사
    // 직전으로 주기를 당기는 압축 스프린트는 팬 실망·충성도를 갉아 투표
    // 지표를 깎아 오히려 대상을 잃었다. 지금은 디지털 지표가 연중 차트
    // 누적을 읽지만, 달력의 이득은 여전히 사각지대(50~52주차 발매) 회피가
    // 중심이라 시드에 따라 우위가 0일 수 있다. 그래서 이 테스트는 "계획이
    // 이긴다"가 아니라 "계획이 손해를 만들면 안 된다"를 하한으로 고정한다.
