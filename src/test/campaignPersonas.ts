...
/**
 * 시상식 역산용 상수. 시상은 매년 50주차(weeksPerYear - 2)에 열리고, 품질
 * 축은 "그 해 1주차~시상 주 사이의 발매작"만, 차트 축은 그 해 1~49주차의
 * 누적 득점만 읽는다. 컴백 시작부터 발매까지 12주
 * (COMEBACK_REQUIREMENTS.releaseWeek)가 걸리므로:
 * - 발매가 50~52주차에 떨어지면 품질은 어느 해 시상에도 집계되지 않고,
 *   차트 런도 새해로 넘어간 구간만 이듬해에 실린다(연말 3주 득점은 연초
 *   리셋으로 소멸) — 여전히 피할 가치가 있는 구간.
 * - 발매가 42~49주차에 떨어지면 품질과 차트 런 초반이 집계되고 활동기
 *   화력(public)이 심사 시점과 겹친다 — 컴백을 앞당길 가치가 있는 구간.
 */
...
export function simulateCampaign(
  ...
  for (let turn = 0; turn < GAME_BALANCE.weeksPerYear * 5; turn++) {
    ...
    const cadenceReady =
      profile === "expert"
        ? awardPlanning
          ? // 시상식 역산: 발매가 50~52주차로 떨어질 타이밍이면 몇 주
            // 미루고 연초 발매로 넘긴다 — 품질 축에서 통째로 빠지고 차트
            // 런의 연말 득점도 연초 리셋으로 소멸하는 구간이다. 심사
            // 직전(42~49주차)으로 주기를 당기는 압축 스프린트는 프로브
            // 결과 오히려 해로웠다 — 사이클 압축이 팬 실망·충성도를 갉아
            // 투표 지표(MAMA 30%)를 깎기 때문. 차트 축 도입 후에도 달력의
            // 이득은 이 낭비 회피가 중심이다.
